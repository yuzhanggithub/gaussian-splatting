...
class GaussianModel:
    ...
    def update_learning_rate(self, iteration):
        ...
        for param_group in self.optimizer.param_groups:
            #??? Will this influence our algorithm?
            # 找到名为"xyz"的参数组，即3D高斯分布中心位置的参数
            if param_group["name"] == "xyz":
                #??? lr is only based on xyz?
    ...
    def reset_opacity(self):
        ...
        """
        #???what would be the issue with low opacity?
    ...
    def replace_tensor_to_optimizer(self, tensor, name):
        ...
        :return: 包含已更新参数的字典。
        """
        #??? why we need this function? video explanation not clear? For deep learning network the variables needs to remain consistent.
        optimizable_tensors = {}
    ...
    def _prune_optimizer(self, mask):
        ...
        #??? what is the difference between gaussian model and optimizer? Similar questions to cat_tensors_to_optimizer
        optimizable_tensors = {}
        for group in self.optimizer.param_groups:
            stored_state = self.optimizer.state.get(group['params'][0], None)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        #??? what does padded means? grads here is a scalar instead of tensor? maybe im not clear with grad here.
