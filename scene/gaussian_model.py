...
class VoxelGaussianArray:
    ...
    def assert_same_dimension(self):
        ...
        assert num_size == self.parent_hash.size(
            0
...
def get_gaussian_array(gaussians: VoxelGaussianArray, sorted_order):
    return VoxelGaussianArray(
        ...
        parent_indices=gaussians.parent_hash[sorted_order],
...
class GaussianModel:
    ...
    def get_root_voxel_index(self, query_translation):
        nearest_index = torch.round(query_translation / self.max_voxel_length)
        return nearest_index
    ...
    self, query_level: torch.Tensor, query_translation: torch.Tensor
        ...
        levels = torch.arange(1, num_levels + 1, device=device).view(-1, 1)  # Shape: (num_levels, 1)
        ...
        query_level_broadcast = query_level.view(1, -1).repeat(num_levels, 1)  # Shape: (num_levels, N)

        # Create a mask where query_level >= current level
        cur_voxel_mask = query_level_broadcast >= levels_broadcast  # Shape: (num_levels, N)
        ...
        query_translation_expanded = query_translation.view(1, N, 3).repeat(num_levels, 1, 1)  # Shape: (num_levels, N, 3)
        ...
        levels_flattened = levels_broadcast.view(-1)                  # Shape: (num_levels * N,)
        upper_levels_flattened = upper_levels_broadcast.view(-1)      # Shape: (num_levels * N,)
        query_translation_flattened = query_translation_expanded.view(-1, 3)  # Shape: (num_levels * N, 3)

        # Compute voxel_based_translation_current using the flattened tensors
        voxel_based_translation_current_flattened = self.get_nearest_voxel_center(
            levels_flattened, 
            query_translation_flattened
        ...
        voxel_based_translation_upper_flattened = self.get_nearest_voxel_center(
            upper_levels_flattened, 
            query_translation_flattened
        )  # Shape: (num_levels * N, 3)

        # Reshape back to (num_levels, N, 3)
        voxel_based_translation_current = voxel_based_translation_current_flattened.view(num_levels, N, 3)  
        voxel_based_translation_upper = voxel_based_translation_upper_flattened.view(num_levels, N, 3)  

        # Compute local coordinates within the upper voxel
        local_coords = (
            voxel_based_translation_current - voxel_based_translation_upper
        )  # Shape: (num_levels, N, 3)
        ...
        relative_index = ((bit_x << 2) | (bit_y << 1) | bit_z) + 1  # Shape: (num_levels, N)

        # Compute weights for each level using exponential weighting: 10^(9 - level)
        weights = torch.pow(10, (9 - levels).float()).long()  # Shape: (num_levels, 1)
    ...
    def get_relative_position_index_parent_level(
        ...
        relative_index = (relative_index_position_hash // divisor) % 10  # Shape: (N,)
    ...
    def get_voxel_index_as_hash(self, voxel_index):
        ...
        hash_values = (voxel_index * base).sum(dim=-1)
        return hash_values

    def get_combined_hash(self, root_voxel_hashes, relative_position_index_hash):
        return (root_voxel_hashes << 32) | relative_position_index_hash
    ...
    def get_root_gaussian_orders_by_root_voxel_hashes(self, query_root_voxel_hashes):
        ...
        query_gaussian_orders = -torch.ones_like(
            query_root_voxel_hashes, dtype=torch.int, device="cuda"
        )
    ...
    def create_voxel_gaussian_arrary(self, raw_scalings, raw_translations):
        array = VoxelGaussianArray()
        array.nearest_voxel_levels = self.get_nearest_voxel_level(raw_scalings)
        ...
        array.parent_hashes = (
            torch.full((num_gaussians,), -1, dtype=torch.long, device="cuda"),
        )
    ...
    def delete_gaussians_and_descendants(
        self, gaussians: VoxelGaussianArray, hashes_to_delete: torch.Tensor
    ) -> VoxelGaussianArray:
        """
        Deletes Gaussians with specified combined_hash values and all their descendants.

        Descendants are defined as Gaussians with combined_hash in the range [H + 1, H + 999] for each hash H to delete.

        Args:
            gaussians (VoxelGaussianArray): The input GaussianArray to modify.
            hashes_to_delete (torch.Tensor): Tensor of shape (M,) containing the combined_hash values to delete.

        Returns:
            VoxelGaussianArray: A new GaussianArray with specified Gaussians and their descendants removed.
        """
        ...
        device = gaussians.combined_hash.device
        N = gaussians.combined_hash.size(0)

        # Step 1: Sort the combined_hash and get sorted gaussians
        sorted_combined_hash, sorted_order = torch.sort(gaussians.combined_hash)
        sorted_gaussians = gaussians[sorted_order]

        # Step 2: Sort the hashes_to_delete
        sorted_delete_hashes, _ = torch.sort(hashes_to_delete)

        # Step 3: Define the deletion ranges [H, H + 999] for each H in hashes_to_delete
        H = sorted_delete_hashes
        H_plus = H + X

        # Step 4: Find start and end indices for each deletion range using searchsorted
        # searchsorted requires sorted tensors
        start_indices = torch.searchsorted(sorted_combined_hash, H, right=False)
        end_indices = torch.searchsorted(sorted_combined_hash, H_plus, right=True)

        # Clamp indices to valid range to avoid out-of-bound errors
        start_indices = torch.clamp(start_indices, max=N)
        end_indices = torch.clamp(end_indices, max=N)

        # Step 5: Create a delta tensor to mark the start and end of deletion ranges
        delta = torch.zeros_like(sorted_combined_hash, dtype=torch.int32, device=device)

        # Increment at start_indices
        delta[start_indices] += 1
        # Decrement at end_indices
        delta[end_indices] -= 1

        # Step 6: Compute the cumulative sum to identify all hashes within any deletion range
        cum_sum = torch.cumsum(delta, dim=0)
        delete_mask_sorted = cum_sum > 0  # True for hashes to delete

        # Step 7: Create a mask for Gaussians to keep (not deleted)
        keep_mask_sorted = ~delete_mask_sorted

        # Step 8: Select the indices to keep
        keep_indices_sorted = keep_mask_sorted.nonzero(as_tuple=False).squeeze()

        # Handle the case where only one element is kept
        if keep_indices_sorted.dim() == 0:
            keep_indices_sorted = keep_indices_sorted.unsqueeze(0)

        # Step 9: Select the Gaussians to keep
        merged_gaussians_sorted = sorted_gaussians[keep_indices_sorted]

        # Step 10: Optionally, reorder back to the original order if needed
        # For simplicity, we return the sorted and filtered gaussians
        return merged_gaussians_sorted

    def build_parent_child_relationships(self, unique_gaussians: VoxelGaussianArray):
        for level in range(
            self.max_voxel_level, 0, -1
        ):  # Iterate from max_voxel_level down to 1
            # Mask for current level gaussians (children)
            cur_children_mask = unique_gaussians.nearest_voxel_levels == level
            children_indices = cur_children_mask.nonzero(as_tuple=False).squeeze()

            if children_indices.numel() == 0:
                continue  # No gaussians at this level

            # Compute required parent hashes
            required_parent_levels = (
                unique_gaussians.nearest_voxel_levels[children_indices] - 1
            )  # Parent level

            required_parent_combined_hash = unique_gaussians.combined_hash // 10 * 10

            # Find unique required parent hashes
            unique_required_parent_combined_hash, inverse_indices = torch.unique(
                required_parent_combined_hash, return_inverse=True
            )

            # Mask for existing parents at level -1
            cur_parent_mask = unique_gaussians.nearest_voxel_levels == (level - 1)
            existing_parent_combined_hash = unique_gaussians.combined_hash[
                cur_parent_mask
            ]
            existing_parent_indices = cur_parent_mask.nonzero(as_tuple=False).squeeze()

            # Sort existing parents' combined_hash for efficient searching
            sorted_existing_parent_hash, sorted_order = torch.sort(
                existing_parent_combined_hash
            )
            sorted_existing_parent_indices = existing_parent_indices[sorted_order]

            # Find which required parent hashes already exist
            # Use torch.searchsorted for efficient matching
            insert_indices = torch.searchsorted(
                sorted_existing_parent_hash, unique_required_parent_combined_hash
            )

            # Check if the found indices are valid and match the required hashes
            valid_existing_parent_mask = (
                insert_indices < sorted_existing_parent_hash.size(0)
            ) & (
                sorted_existing_parent_hash[insert_indices]
                == unique_required_parent_combined_hash
            )

            # Parents that already exist
            existing_parents = sorted_existing_parent_indices[
                insert_indices[valid_existing_parent_mask]
            ]
            # Children that have existing parents
            children_with_existing_parents = children_indices[
                valid_existing_parent_mask[inverse_indices]
            ]

            # Assign parents to these children
            unique_gaussians.parent_hashes[
                children_with_existing_parents
            ] = existing_parents

            # Assign children to parents based on relative_position_index
            rel_pos_existing = unique_gaussians.relative_position_index[
                children_with_existing_parents
            ]
            # Ensure rel_pos_existing is within 0-7
            assert (
                (rel_pos_existing >= 0) & (rel_pos_existing < 8)
            ).all(), "relative_position_index out of bounds"

            unique_gaussians.children_hashes[
                existing_parents, rel_pos_existing
            ] = children_with_existing_parents

            # Handle children without existing parents (need to create new parents)
            children_without_existing_parents = children_indices[
                ~valid_existing_parent_mask[inverse_indices]
            ]
            unique_new_parent_combined_hash = required_parent_combined_hash[
                ~valid_existing_parent_mask[inverse_indices]
            ]

            if children_without_existing_parents.numel() == 0:
                continue  # All parents exist, no new parents needed

            # Find unique new parent hashes and group children accordingly
            unique_new_parent_combined_hash, new_inverse_indices = torch.unique(
                unique_new_parent_combined_hash, return_inverse=True
            )
            num_new_parents = unique_new_parent_combined_hash.size(0)

            # For each new parent, find children
            # Compute sums for translations, scales, and colors
            sum_translations = torch.zeros((num_new_parents, 3), device="cuda")
            sum_scales = torch.zeros((num_new_parents,), device="cuda")
            sum_colors = torch.zeros(
                (num_new_parents, unique_gaussians.color.size(1)), device="cuda"
            )
            ...
            sum_scales.index_add_(
                0,
                new_inverse_indices,
                unique_gaussians.voxel_based_scales[children_without_existing_parents],
            )
            ...
            # Compute averages
            counts_new_parents = (
                torch.bincount(new_inverse_indices, minlength=num_new_parents)
                .unsqueeze(1)
                .float()
            )
            avg_translations = sum_translations / counts_new_parents
            avg_scales = sum_scales / counts_new_parents.squeeze()
            avg_colors = (sum_colors / counts_new_parents).round().long()

            # Decode new parent combined_hash to get root_voxel_hash and relative_position_index_hash
            (
                new_parent_root_voxel_hash,
                new_parent_relative_position_index_hash,
            ) = self.decode_combined_hash(unique_new_parent_combined_hash)

            # Create new parent Gaussians
            new_parent_gaussians = VoxelGaussianArray(
                combined_hash=unique_new_parent_combined_hash,
                root_voxel_hash=new_parent_root_voxel_hash,
                relative_position_index_hash=new_parent_relative_position_index_hash,
                nearest_voxel_levels=torch.full(
                    (num_new_parents,), level - 1, dtype=torch.long, device="cuda"
                ),
                relative_position_index=torch.full(
                    (num_new_parents,), 8, dtype=torch.long, device="cuda"
                ),  # Set to 8 as per requirement
                voxel_based_translations=avg_translations,
                voxel_based_scales=avg_scales,
                color=avg_colors,
                parent_hashes=torch.full(
                    (num_new_parents,), -1, dtype=torch.long, device="cuda"
                ),
                children_hashes=torch.full(
                    (num_new_parents, 8), -1, dtype=torch.long, device="cuda"
                ),
            )

            # Append new parents to unique_gaussians
            unique_gaussians = cat_gaussian_array(
                unique_gaussians, new_parent_gaussians
            )

            # Update parent_indices for children without existing parents
            # New parents are at indices from original N to N + num_new_parents
            original_N = unique_gaussians.combined_hash.size(0) - num_new_parents
            new_parent_indices = torch.arange(
                original_N, original_N + num_new_parents, device="cuda"
            )

            # TODO: this doesn't look correct. required_parent_levels may be wrong
            # Assign children to new parents based on their relative_position_index
            rel_pos_new = self.get_relative_position_index_parent_level(
                unique_gaussians.combined_hash[children_without_existing_parents],
                required_parent_levels,
            )
            # Since relative_position_index for parents is set to 8, but children have their own relative_position_index
            # Assign based on children's relative_position_index
            unique_gaussians.children_hashes[new_parent_indices, :] = -1  # Initialize

            # Assign children to their respective new parents
            unique_gaussians.parent_hashes[
                children_without_existing_parents
            ] = new_parent_indices[new_inverse_indices]
            unique_gaussians.children_hashes[
                new_parent_indices[new_inverse_indices],
                unique_gaussians.relative_position_index[
                    children_without_existing_parents
                ],
            ] = children_without_existing_parents
    ...
    def create_from_pcd(
        ...
        num_of_gaussians = fused_translation.shape[0]
        ...
        voxel_based_scales = torch.tensor(
            [self.get_voxel_sphere_radius(level) for level in nearest_voxel_levels]
        ).cuda()
        voxel_based_scales = torch.log(voxel_based_scales)[..., None]

        voxel_based_translation = self.get_nearest_voxel_center(
            nearest_voxel_levels, fused_translation
        )
        # Compute root voxel hashes
        root_voxel_hash = self.get_voxel_index_as_hash(
            self.get_nearest_root_voxel_index(voxel_based_translation)
        )
        # Compute relative position indices
        relative_position_index = (
            self.get_nearest_relative_position_index_to_upper_voxel_level(
                nearest_voxel_levels, voxel_based_translation
            )
        )

        # Verify tensor shapes
        assert (
            root_voxel_hash.shape
            == nearest_voxel_levels.shape
            == relative_position_index.shape
        ), f"Shapes mismatch: root_voxel_hash {root_voxel_hash.shape}, nearest_voxel_levels {nearest_voxel_levels.shape}, relative_position_index {relative_position_index.shape}"

        unique_gaussian_array = self.sort_and_deduplicate_voxels(
            VoxelGaussianArray(
                root_voxel_hash,
                nearest_voxel_levels,
                relative_position_index,
                voxel_based_translation,
                voxel_based_scales,
                fused_color,
            )
        )

        full_gaussian_array = self.build_parent_child_relationships(
            unique_gaussian_array
        )
        ...
        # 将以上计算的参数设置为模型的可训练参数
        self.combined_hash = full_gaussian_array.combined_hash
        self.root_voxel_hash = full_gaussian_array.root_voxel_hash
        self.relative_position_index_hash = (
            full_gaussian_array.relative_position_index_hash
        )
        self.voxel_level = full_gaussian_array.nearest_voxel_levels
        self.relative_position_index = full_gaussian_array.relative_position_index
        self.parent_hash = full_gaussian_array.parent_hash
        self.children_hashes = full_gaussian_array.children_hashes

        self._xyz = nn.Parameter(
            full_gaussian_array.voxel_based_translations.requires_grad_(True)
        )
        ...
        self._scaling = nn.Parameter(
            full_gaussian_array.voxel_based_scales.requires_grad_(True)
        )
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        big_pts_mask = torch.where(padded_grad >= grad_threshold, True, False)
        big_pts_mask = torch.logical_and(
            selected_pts_mask,
        ...
        selected_pts_mask = torch.logical_and(big_pts_mask, voxel_level_mask)
        # print("execute here3")
    ...
    def try_merge_new_points(
        ...
        insert_indices = torch.searchsorted(sorted_existing_hash, new_hash)

        # Check if the inserted indices have matching hashes
        matches = (insert_indices < sorted_existing_hash.size(0)) & (
            sorted_existing_hash[insert_indices] == new_hash
        )

        # Indices in existing_gaussians that match
        existing_match_indices = sorted_indices[insert_indices[matches]]
